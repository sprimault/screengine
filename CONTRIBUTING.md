...
- A dependency that ships in a published archive goes into
  `THIRD-PARTY-NOTICES` — and in the core, it does not go in at all. The file
  also covers the Rust standard library, linked into every library: whenever
  `rust-version` is raised, reread the dependencies in
  `library/std/Cargo.toml` and carry over what changed.
