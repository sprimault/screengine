...
- **Deux démonstrations de bureau, en C et en C++.** Elles chargent la carte et
  ...
  le moteur. Hors de tout contrôle, comme tout ce qui ouvre une fenêtre :
  `make demo-c` et `make demo-cpp` les lancent.
- **La page web parcourt un décor.** Elle charge `couloir.world` et
  `caisse.mesh` — versionnés, engendrés par la conformance —, dessine par tuiles
  le couloir et les trois caisses qui y sont posées, et s'y déplace au clavier.
  C'est l'hôte de démonstration du web, celui qu'un
  intégrateur copie pour partir : canvas, événements, boucle, et rien d'autre.
...
- **Two desktop demonstrations, in C and in C++.** They load the versioned map
  ...
  engine. Outside every check, like anything that opens a window: `make demo-c`
  and `make demo-cpp` run them.
- **The web page walks through a decor.** It loads `couloir.world` and
  `caisse.mesh` — versioned and generated by the conformance suite — draws the
  corridor and the three crates standing in it by tiles, and moves around with
  the keyboard. It is the web demonstration host, the one an
  integrator copies to start from: a canvas, events, a loop, and nothing else.
