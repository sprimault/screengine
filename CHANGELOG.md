...
- Les conventions Rust décrivent les dispositions binaires des deux formats :
  ...
  tient pour hostile.
- **La page web parcourt un décor.** Elle charge `couloir.world` — versionné,
  engendré par la conformance comme le maillage —, le dessine par tuiles et s'y
  déplace au clavier. C'est l'hôte de démonstration du web, celui qu'un
...
- **The web page walks through a decor.** It loads `couloir.world` — versioned
  and generated by the conformance suite, like the mesh — draws it by tiles and
  moves around with the keyboard. It is the web demonstration host, the one an
